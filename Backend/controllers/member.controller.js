...
// @access  Private (Staff only)
const toggleDoorAccess = asyncHandler(async (req, res, next) => {
...
// @access  Private (Staff only)
const addMemberNote = asyncHandler(async (req, res, next) => {
  const { note } = req.body;
  const member = await Member.findById(req.params.id);
  if (!member) {
    return res.status(404).json(generateResponse(false, "Member not found", null));
...
// @access  Private (Staff only)
const getOverdueMembers = asyncHandler(async (req, res, next) => {
  const now = new Date();
